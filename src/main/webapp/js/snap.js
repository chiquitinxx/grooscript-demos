...
SnapSvg.snapSvg("svg", function(it) {
  ...
  (59).times(function(val) {
    return gs.sp(gs.mc(gs.mc(secondPoint,"copy",[]),"rotate",[gs.multiply(6, (gs.plus(val, 1))), gs.gp(middle,"x"), gs.gp(middle,"y")]),"r",(gs.mod((gs.plus(val, 1)), 5) ? 2 : 3));
  });
  ...
  var rotateClockHands = function(it) {
    ...
    var hour = gs.mod((nowValues[0]), 12);
    var minute = nowValues[1];
    gs.mc(hours,"rotate",[gs.plus((gs.multiply(hour, 30)), (gs.div(minute, 2))), gs.gp(middle,"x"), gs.gp(middle,"y")]);
  ...
  gs.execCall(rotateClockHands, this, []);
