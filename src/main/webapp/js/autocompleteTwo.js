...
function ReactiveScript() {
  ...
  gSobject.searchWikipedia = function(term) {
    return gs.mc(gs.mc(gs.fs('jQuery', this, gSobject),"ajax",[gs.map().add("url","http://en.wikipedia.org/w/api.php").add("dataType","jsonp").add("data",gs.toJavascript(gs.map().add("action","opensearch").add("format","json").add("search",gs.mc(gs.fs('window', this, gSobject),"encodeURI",[term]))))]),"promise",[]);
  };
